...
public class TransactionService {
    ...
    private void addActualReceivedQuantityToReceiver(Transaction transaction, TransactionItem item, int receivedQuantity) {
        ...
        if (transaction.getReceiverType() == PartyType.WAREHOUSE) {
            addToWarehouseInventory(transaction.getReceiverId(), item.getItemType(), receivedQuantity);
    ...
    private void createStolenItemEntry(Transaction transaction, TransactionItem item, int stolenQuantity) {
        ...
        if (transaction.getSenderType() == PartyType.WAREHOUSE) {
            ...
            Item stolenItem = new Item(item.getItemType(), warehouse, stolenQuantity, ItemStatus.STOLEN);
            ...
            stolenConsumable.setStatus(ItemStatus.STOLEN);
    ...
    private void createOverReceivedItemEntry(Transaction transaction, TransactionItem item, int overReceivedQuantity) {
        ...
        if (transaction.getReceiverType() == PartyType.WAREHOUSE) {
            ...
            Item overReceivedItem = new Item(item.getItemType(), warehouse, overReceivedQuantity, ItemStatus.OVERRECEIVED);
    ...
    private void addToWarehouseInventory(UUID warehouseId, ItemType itemType, int quantity) {
        ...
        if (!existingItems.isEmpty()) {
            ...
            Item newItem = new Item(itemType, warehouse, quantity, ItemStatus.IN_WAREHOUSE);
            itemRepository.save(newItem);
        }
        System.out.println("✅ Added " + quantity + " to warehouse inventory");
    }
